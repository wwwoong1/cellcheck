...
class CameraUVBox(Node):
    def __init__(self):
        ...
        # Joint States Publisher 추가
        self.joint_states_publisher = self.create_publisher(
            JointState,
            'arduino_joint_states',
            10
        )
    ...
    def publish_joint_states(self):
        ...
        joint_state_msg.name = ['base_to_servo1', 'upper_arm_to_servo2', 'forearm_to_servo3', 'gripper_control']
        
        if self.appearance_class == 0:  # AAA(집게로 인해 불가능)
            joint_state_msg.position = [74.4845, 62.4524, 90.0, 34.3775]
        elif self.appearance_class == 1:  # AA
            joint_state_msg.position = [72.11972, 59.58761, 90.0, 26.35606]
        elif self.appearance_class == 2:  # C
            joint_state_msg.position = [84.2248, 77.3493, 90.0, 27.215495]
        elif self.appearance_class == 3:  # D
            joint_state_msg.position = [82.000, 76.000, 90.0, 27.0]
            
        self.joint_states_publisher.publish(joint_state_msg)
        self.get_logger().info(f'Published joint states for appearance_class: {self.appearance_class}')
